...
const KATAKANA_DATA = {
  basic: [
    { char: 'ア', romaji: 'a', type: 'vowel' }, { char: 'イ', romaji: 'i', type: 'vowel' }, { char: 'ウ', romaji: 'u', type: 'vowel' }, { char: 'エ', romaji: 'e', type: 'vowel' }, { char: 'オ', romaji: 'o', type: 'vowel' },
    { char: 'カ', romaji: 'ka', type: 'k-row' }, { char: 'キ', romaji: 'ki', type: 'k-row' }, { char: 'ク', romaji: 'ku', type: 'k-row' }, { char: 'ケ', romaji: 'ke', type: 'k-row' }, { char: 'コ', romaji: 'ko', type: 'k-row' },
    { char: 'サ', romaji: 'sa', type: 's-row' }, { char: 'シ', romaji: 'shi', type: 's-row' }, { char: 'ス', romaji: 'su', type: 's-row' }, { char: 'セ', romaji: 'se', type: 's-row' }, { char: 'ソ', romaji: 'so', type: 's-row' },
    { char: 'タ', romaji: 'ta', type: 't-row' }, { char: 'チ', romaji: 'chi', type: 't-row' }, { char: 'ツ', romaji: 'tsu', type: 't-row' }, { char: 'テ', romaji: 'te', type: 't-row' }, { char: 'ト', romaji: 'to', type: 't-row' },
    { char: 'ナ', romaji: 'na', type: 'n-row' }, { char: 'ニ', romaji: 'ni', type: 'n-row' }, { char: 'ヌ', romaji: 'nu', type: 'n-row' }, { char: 'ネ', romaji: 'ne', type: 'n-row' }, { char: 'ノ', romaji: 'no', type: 'n-row' },
    { char: 'ハ', romaji: 'ha', type: 'h-row' }, { char: 'ヒ', romaji: 'hi', type: 'h-row' }, { char: 'フ', romaji: 'fu', type: 'h-row' }, { char: 'ヘ', romaji: 'he', type: 'h-row' }, { char: 'ホ', romaji: 'ho', type: 'h-row' },
    { char: 'マ', romaji: 'ma', type: 'm-row' }, { char: 'ミ', romaji: 'mi', type: 'm-row' }, { char: 'ム', romaji: 'mu', type: 'm-row' }, { char: 'メ', romaji: 'me', type: 'm-row' }, { char: 'モ', romaji: 'mo', type: 'm-row' },
    { char: 'ヤ', romaji: 'ya', type: 'y-row' }, { char: null }, { char: 'ユ', romaji: 'yu', type: 'y-row' }, { char: null }, { char: 'ヨ', romaji: 'yo', type: 'y-row' },
    { char: 'ラ', romaji: 'ra', type: 'r-row' }, { char: 'リ', romaji: 'ri', type: 'r-row' }, { char: 'ル', romaji: 'ru', type: 'r-row' }, { char: 'レ', romaji: 're', type: 'r-row' }, { char: 'ロ', romaji: 'ro', type: 'r-row' },
    { char: 'ワ', romaji: 'wa', type: 'w-row' }, { char: null }, { char: null }, { char: null }, { char: 'ヲ', romaji: 'wo', type: 'w-row' },
    { char: 'ン', romaji: 'n', type: 'n' }
  ],
  dakuten: [
    { char: 'ガ', romaji: 'ga' }, { char: 'ギ', romaji: 'gi' }, { char: 'グ', romaji: 'gu' }, { char: 'ゲ', romaji: 'ge' }, { char: 'ゴ', romaji: 'go' },
    { char: 'ザ', romaji: 'za' }, { char: 'じ', romaji: 'ji' }, { char: 'ズ', romaji: 'zu' }, { char: 'ぜ', romaji: 'ze' }, { char: 'ぞ', romaji: 'zo' },
    { char: 'ダ', romaji: 'da' }, { char: 'ぢ', romaji: 'ji' }, { char: 'づ', romaji: 'zu' }, { char: 'で', romaji: 'de' }, { char: 'ど', romaji: 'do' },
...
export default function KatakanaPage() {
  ...
  const allCharsManual = useMemo(() => {
    return [...KATAKANA_DATA.basic, ...KATAKANA_DATA.dakuten, ...KATAKANA_DATA.handakuten, ...KATAKANA_DATA.yoon].filter(item => item && item.char);
  }, []);

  const quizPool = useMemo(() => {
    if (difficulty === 'easy') return KATAKANA_DATA.basic.filter(i => i?.char);
    if (difficulty === 'mid') return [...KATAKANA_DATA.basic, ...KATAKANA_DATA.dakuten, ...KATAKANA_DATA.handakuten].filter(i => i?.char);
    return allCharsManual;
  }, [difficulty, allCharsManual]);

  // Persistence Logic (separated storage for Katakana)
  useEffect(() => {
    const saved = localStorage.getItem('katakana_stats');
    if (saved) {
  ...
  const updateStats = useCallback(() => {
    const newStats = {
      daily: (stats.daily || 0) + 1,
      weekly: (stats.weekly || 0) + 1,
      lastCheck: new Date().toISOString()
    };
    setStats(newStats);
    localStorage.setItem('katakana_stats', JSON.stringify(newStats));
  }, [stats]);
    ...
    <div className="bg-[#fafafa] dark:bg-black min-h-screen text-black dark:text-white transition-colors duration-500 overflow-hidden">
      ...
      <div className="container mx-auto px-6 pt-32 pb-32 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-8">
              ...
              {['easy', 'mid', 'hard'].map(d => (
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${
                    difficulty === d ? 'bg-orange-500 border-orange-500 text-white shadow-lg' : 'border-black/5 dark:border-white/5 opacity-40'
                  }`}
                >
                  {d}
                </button>
           ...
           <div className="flex gap-6">
              <div className="flex items-center gap-2 opacity-60">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest">Day: {stats.daily || 0}</span>
              </div>
              <div className="flex items-center gap-2 opacity-60">
                 <FaCalendarWeek className="text-blue-500" />
                 <span className="text-[10px] font-black uppercase tracking-widest">Week: {stats.weekly || 0}</span>
              </div>
           </div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto text-center mb-16">
          <div className="inline-block px-4 py-1 rounded-full bg-orange-500/10 border border-orange-500/20 text-orange-500 text-[10px] font-black uppercase tracking-widest mb-6">Katakana Pro</div>
          <h1 className="text-6xl md:text-8xl font-black mb-6 tracking-tighter font-noto">カタカナ <span className="text-gray-300 dark:text-neutral-800">Learning</span></h1>
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="grid" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.02 }}>
                <div className="flex gap-4 mb-10 justify-center">
                   {['basic', 'dakuten', 'handakuten', 'yoon'].map(group => (
                     <button key={group} onClick={() => setSelectedGroup(group)} className={`text-[10px] font-black uppercase tracking-widest pb-2 border-b-2 transition-all ${selectedGroup === group ? 'border-orange-500 text-orange-500' : 'border-transparent opacity-30'}`}>{group}</button>
                   ))}
                </div>
                <div className="grid grid-cols-5 md:grid-cols-10 gap-2 md:gap-3">
                   {KATAKANA_DATA[selectedGroup].map((item, idx) => item ? (
                     <motion.div key={idx} whileHover={{ scale: 1.05, y: -2 }} onClick={() => playSound(item.char)} className="aspect-square flex flex-col items-center justify-center rounded-xl bg-white dark:bg-neutral-900 border border-black/5 dark:border-white/5 shadow-sm hover:shadow-xl hover:border-orange-500 group cursor-pointer p-2">
                       <span className="text-2xl font-noto font-black group-hover:text-orange-500 transition-colors">{item.char}</span>
                ...
                <div className="w-72 h-[400px] relative perspective-1000 cursor-pointer group" onClick={() => setIsFlipped(!isFlipped)}>
                  <motion.div animate={{ rotateY: isFlipped ? 180 : 0 }} transition={{ type: 'spring', damping: 20, stiffness: 100 }} className="w-full h-full relative preserve-3d">
                    <div className="absolute inset-0 backface-hidden bg-white dark:bg-neutral-900 rounded-[2.5rem] border border-black/5 dark:border-white/5 shadow-22 flex flex-col items-center justify-center gap-6">
                       ...
                       <button onClick={(e) => { e.stopPropagation(); playSound(allCharsManual[activeIndex].char); }} className="p-3 rounded-full bg-gray-100 dark:bg-neutral-800 hover:scale-110 transition-transform"><FaVolumeUp className="text-orange-500" /></button>
                    </div>
                    <div className="absolute inset-0 backface-hidden bg-orange-500 rounded-[2.5rem] flex flex-col items-center justify-center gap-2 text-white" style={{ transform: 'rotateY(180deg)' }}>
            ...
            {mode === 'results' && (
              <motion.div key="results" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center text-center">
                 <div className="w-32 h-32 rounded-full bg-orange-500/10 flex items-center justify-center mb-8 border border-orange-500/20">
                    <FaTrophy className="text-6xl text-orange-500" />
                 </div>
                 <h2 className="text-4xl md:text-6xl font-black mb-4 tracking-tighter">Mission <span className="text-orange-500">Complete</span></h2>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-2xl mb-16">
                    ...
                    <div className="p-8 rounded-[2.5rem] bg-white dark:bg-neutral-900 border border-black/5 flex items-center gap-6">
                       ...
                       <div className="text-left">
                          ...
                          <div className="text-3xl font-black">{stats.daily || 0}/5</div>
                       </div>
                    </div>
                 </div>
